...
(defvar *gmail-username* "YOURUSERNAME"
  "Username for gmail")

(defvar *gmail-password* "YOURPASSWORD"
  "Password for gmail")

(load "~/Dropbox/Lisp/kennwörter.lisp") ; here I saved my *gmail-username* and *gmail-password* separately
...
(defun format-gmail (stream)
  ...
  (cond
    ((null *gmail-last-value*)
     (format stream "[^Bmail:???^b]"))
    ((and (numberp *gmail-last-value*)
      (zerop *gmail-last-value*))
     (format stream "[^Bmail:^b 0]"))
    ((numberp *gmail-last-value*)
     (format stream "[^BMAIL:~2D^b]" *gmail-last-value*))
    (t
     (format stream "[^Bmail:ERR^b]"))))
